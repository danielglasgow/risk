...
public class StandardBattlePredictor implements BattlePredictor {
	...
	private void buildStartingValues() {
		outcomes.put(new DiscreteBattle(2, 1), WIN_PERCENTAGE_ONE_DIE_VS_ONE
				* 2 + LOSE_PERCENTAGE_ONE_DIE_VS_ONE * 1);
		outcomes.put(
				new DiscreteBattle(3, 1),
	...
	public double predict(Battle battle) {
		...
		double outcome = predictDiscreteBattle(new DiscreteBattle(attackArmies,
				defenseArmies));
	...
	private double predictOutcome(DiscreteBattle discreteBattle) {
		...
		if (discreteBattle.attackArmies > 3
				&& discreteBattle.defenseArmies == 1) {
